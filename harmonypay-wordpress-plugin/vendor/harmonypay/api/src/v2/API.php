...
abstract class API
{
	...
	**/
	public static $api_url = 'http://api.harmonypay.test/api/v1/';
	...
	**/
	...
	/**
		@brief		Return the donations component.
		@since		2019-03-06 23:34:51
	**/
	public function donations()
	{
		...
		$this->__donations = $this->new_donations();
		return $this->__donations;
	}
	...
	**/
	...
	{
		if ( defined( 'HARMONYPAY_PRICING_URL' ) )
			$url = HARMONYPAY_PRICING_URL;
		else
			$url = 'https://harmonypay.com/pricing/';
	...
	**/
	...
	/**
	@brief		Create the donations component.
	@since		2019-03-06 23:35:37
	**/
	...
	**/
	public function process_messages( $json )
	{
		...
		if ( $account->get_domain_key() != $json->harmonypay )
			return $this->debug( 'Invalid domain key. Received %s', $json->harmonypay );
		...
		foreach( $json->messages as $message )
		{
			...
			switch( $message->type )
			{
				...
				case 'cancel_payment':
					...
					break;
				case 'refund_payment':
					// Mark the local payment as refunded.
					$this->payments()->refund_local( $payment );
					break;					
