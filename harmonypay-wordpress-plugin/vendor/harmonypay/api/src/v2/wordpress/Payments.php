...
namespace harmonypay\api\v2\wordpress;

use harmonypay\api\v2\Exception;
...
**/
class Payments
	extends \harmonypay\api\v2\Payments
{
	/**
	...
	**/
	public static function add_to_checkout_javascript_data( $action, \harmonypay\api\v2\Payment $payment )
	{
	...
	**/
	public function cancel_local( \harmonypay\api\v2\Payment $payment )
	{
		$this->do_local( 'cancel_payment', $payment );
	}

	/**
		@brief		Refund a local payment.
		@since		2021-07-30 18:23:18
	**/
	public function refund_local( \harmonypay\api\v2\Payment $payment )
	{
		$this->do_local( 'refund_payment', $payment );
	}
	...
	**/
	public function complete_local( \harmonypay\api\v2\Payment $payment )
	{
	...
	**/
	public function do_local( $message_type, \harmonypay\api\v2\Payment $payment )
	{
		$action = HarmonyPay()->new_action( $message_type );
	...
	**/
	public static function generate_payment_from_order( $post_id )
	{
		...
		$payment->amount = get_post_meta( $post_id,  '_hrp_amount', true );
		$payment->confirmations = get_post_meta( $post_id,  '_hrp_confirmations', true );
		$payment->created_at = get_post_meta( $post_id,  '_hrp_created_at', true );
		$payment->currency_id = get_post_meta( $post_id,  '_hrp_currency_id', true );
		$payment->timeout_hours = get_post_meta( $post_id,  '_hrp_payment_timeout_hours', true );
		$payment->to = get_post_meta( $post_id,  '_hrp_to', true );

		$payment->data = get_post_meta( $post_id,  '_hrp_payment_data', true );
	...
	**/
	...
	{
		update_post_meta( $post_id, '_hrp_amount', $payment->amount );
		update_post_meta( $post_id, '_hrp_confirmations', $payment->confirmations );
		update_post_meta( $post_id, '_hrp_created_at', $payment->created_at );
		update_post_meta( $post_id, '_hrp_currency_id', $payment->currency_id );
		update_post_meta( $post_id, '_hrp_payment_timeout_hours', $payment->timeout_hours );
		update_post_meta( $post_id, '_hrp_to', $payment->to );
		update_post_meta( $post_id, '_hrp_payment_data', $payment->data );
	}
	...
	**/
	...
	{
		$attempts = intval( get_post_meta( $post_id, '_hrp_attempts', true ) );

		HarmonyPay()->api()->account()->lock()->save();

		try
		{
			$payment = static::generate_payment_from_order( $post_id );
			$payment_id = $this->add( $payment );
			update_post_meta( $post_id, '_hrp_payment_id', $payment_id );
			$this->api()->debug( 'Payment for order %d has been added as payment #%d.', $post_id, $payment_id );
		}
		catch ( Exception $e )
		{
			$attempts++;
			update_post_meta( $post_id, '_hrp_attempts', $attempts );
			...
			if ( $attempts > 24 * 60 )	// 24 hours, 60 times an hour, since this is usually run on the hourly cron.
			{
				...
				update_post_meta( $post_id,  '_hrp_payment_id', -1 );
			}
			else
			{
				// Try again in 5 minutes.
				wp_schedule_single_event( time() + 60, 'harmonypay_send_payment', [ $post_id, microtime() ] );
	...
	**/
	public function send_unsent_payments()
	{
		...
		global $wpdb;
		$query = sprintf( "SELECT `post_id` FROM `%s` WHERE `meta_key` = '_hrp_payment_id' AND `meta_value` = '0'",
			$wpdb->postmeta
		);
