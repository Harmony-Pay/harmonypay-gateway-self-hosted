	...
	extends Component
{
	...
	**/
	public function is_available_for_payment()
	{
		...
		//	throw new Exception( 'Your account does not have any payments left this month. Either wait until next month or purchase an unlimited license using the link on your HarmonyPay settings account page.' );
	...
	**/
	...
	{
		//HarmonyPay()->debug( 'this->data->domain_key: %s', $this->data );
	...
	**/
	...
	{
		$this->data = (object)[];
		//HarmonyPay()->debug( 'static::$account_data_site_option_key: %s', static::$account_data_site_option_key );
		$data = $this->api()->get_data( static::$account_data_site_option_key );
		$data = json_decode( $data );
		//HarmonyPay()->debug( '$data: %s', $data );
		if ( ! $data )
			$this->data = (object)[];
		else
			$this->data = $data;
	...
	**/
	public function generate_client_account_data()
	{
		...
		$client_account_data->domain = base64_encode( HarmonyPay()->get_client_url() );
		$client_account_data->plugin_version = HARMONYPAY_PLUGIN_VERSION;
	...
	**/
	public function retrieve()
	{
		try
		{
			...
			//HarmonyPay()->debug( 'Client_account_data: %s', $client_account_data );
			$result = $this->send_client_account_data( $client_account_data );
			//HarmonyPay()->debug( 'Result : %s', $result  );
			$this->api()->save_data( static::$account_data_site_option_key, json_encode( $result->harmonypay->messages[0]->account ) );
			$this->load_data();
			if ( ! $this->is_valid() )
		...
		{
			HarmonyPay()->debug( 'WARNING: Unable to retrieve our account details: %s', $e->get_message() );
			return false;
		}
	}
	...
	**/
	public function send_client_account_data( \harmonypay\api\v2\Client_Account_Data $client_account_data )
	{
		$result = HarmonyPay()->api()->send_post( 'account/retrieve', $client_account_data );
		if ( ! $result )
